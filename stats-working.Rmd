...
H.F.hyacinthfam <- function(x, this_caption, flower) x %>% select(all_of(flower)) %>%
  ...
  kable_styling(latex_options = c("repeat_header", "striped", "hold_position"), font_size = 6) %>%
...
Qualification: three innings.

```{r bat-avgs-thisyear}
E.batAvg.TY %>% filter(club_id == conf$club_of_interest &
                         Inns >= conf$qualify_inns_avgs  &
                         !is.na(Name)) %>% arrange(desc(Avg)) %>% H.F.plain("Batting averages, this year", H.aster)
```

### Most runs

```{r mostruns-thisyear}
E.batAvg.TY %>% filter(club_id == conf$club_of_interest & !is.na(Name)) %>% arrange(desc(Runs)) %>% H.F.plain("Runs scored, this year", H.aster)
```
...
for (i in levels(F.batting.us$Type)) {
  ...
  zxc %>% filter(Type == i) %>%
    ...
    H.F.plain(paste("Most runs in ", i, " games this year"),
              H.mugwort) %>% 
    print()
  cat ('\n\n')
...
E.bowlAvg.TY %>% filter(club_id == conf$club_of_interest & !is.na(Name)) %>% arrange(desc(W), R) %>%
  H.F.plain("Most wickets this year", H.clematis)
```
...
F.inningses.us %>% slice_max(actuallyDate, n=conf$default_list_length) %>% 
  H.F.hyacinthfam("Recent club innings", H.hyacinth) #was dianthus
```
...
B.allround %>% filter(Runs >= 30 & W >= 3 & 
                        ...
                        Yr==conf$year_of_interest) %>% 
  H.F.violetfam("Thirty and three-for", H.violet)
```
...
### Most runs
```{r mostruns-alltime}
B.batAvg %>% filter(is_us == TRUE & !is.na(Name)) %>%
...
F.batting.circle %>% filter(`How Out` %in% unusual_dismissals) %>% arrange(desc(actuallyDate)) %>%
...
B.inningses %>% filter(batting_club_id == conf$club_of_interest,
  ...
  H.F.hyacinthfam("Lowest runs/wkt, our batting", H.hyacinth)
```
...
F.inningses.circle %>% filter(topscore < 100) %>% 
  slice_max(Total, n=conf$default_list_length ) %>%
  H.F.hyacinthfam("Highest totals without any individual century", H.hyacinth)


F.inningses.circle %>% filter(topscore < 50) %>% 
  slice_max(Total, n=conf$default_list_length ) %>%
  H.F.hyacinthfam("Highest totals without any individual fifty", H.hyacinth)
...
F.inningses.circle %>% filter(numcenturies >= 2) %>% 
  arrange(desc(actuallyDate)) %>%
  H.F.hyacinthfam("Innings with two or more centuries", H.hyacinth)

F.inningses.circle %>% filter(numfifties >= 3) %>% 
  arrange(desc(actuallyDate)) %>%
  H.F.hyacinthfam("Innings with three or more fifties", H.hyacinth)
...
F.inningses.circle %>% filter(botscore >= 10  & numbats >= 6) %>% 
  arrange(desc(actuallyDate)) %>%
  H.F.hyacinthfam("Innings with all (6+) batters making 10 or more", H.hyacinth)
```
...
F.inningses.circle %>% filter(Res == "W", topscore != 0,  topscore <= 15) %>% 
  arrange(desc(actuallyDate)) %>%
  H.F.hyacinthfam("Winning with no batter over 15", H.hyacinth)
...
F.inningses.circle %>% filter(numfourfs >= 2) %>% 
  slice_max(actuallyDate, n=conf$default_list_length ) %>%
  H.F.hyacinthfam("Innings with two four-fors", H.hyacinth)

F.inningses.circle %>% filter(minwkts >= 2) %>% 
  slice_max(actuallyDate, n=conf$default_list_length ) %>%
  H.F.hyacinthfam("All bowlers taking at least two wickets", H.hyacinth)
```
...
F.inningses.circle %>% filter(Total > 250 & maxrunsccd < 50) %>%
  arrange(desc(actuallyDate)) %>%
  H.F.hyacinthfam("Innings for 250+ with no bowler conceding 50", H.hyacinth)
```
...
F.inningses.circle %>% filter(BatInnsWinner == 1, Res == "W", Margin >= 250) %>% 
    arrange(desc(actuallyDate)) %>%
    H.F.hyacinthfam("Wins by 250 runs or more", H.phlox)
...
F.inningses.circle %>% filter(BatInnsWinner == 2, Res == "W", Margin == 10) %>% 
    arrange(desc(actuallyDate)) %>%
    H.F.hyacinthfam("Wins by 10 wickets", H.phlox)
...
F.inningses.circle %>% filter(BatInnsWinner == 1, Res == "W", Margin == 1) %>% 
    arrange(desc(actuallyDate)) %>%
    H.F.hyacinthfam("Wins by 1 run", H.phlox)
...
B.allround %>% filter(Runs >= 50 & W >= 4 & 
                        batting_club_id == conf$club_of_interest) %>% 
  arrange(desc(actuallyDate)) %>%
  H.F.violetfam("Fifty and four-for", H.violet)
```
...
B.allround %>% filter(Runs >= 100 & W >= 5 & 
                        is_circle==TRUE) %>% 
  arrange(desc(actuallyDate)) %>%
  H.F.violetfam("Hundred and five-for", H.violet)
```
...
B.allround %>% filter(Runs >= 50 & (Ct+ Std) >= 3 & 
                        batting_club_id == conf$club_of_interest) %>% 
  arrange(desc(actuallyDate)) %>%
  H.F.violetfam("Fifty and three dismissals", H.violet)
```
...
F.inningses.circle %>% group_by(ground_id) %>%
  slice_max(Total) %>% ungroup() %>% drop_na(Ground) %>%
    ...
    H.F.hyacinthfam("Highest team total at grounds", H.phlox)
```
...
for (i in 1:length(conf$opponents_of_interest)) {
  ...
  B.inningses %>% filter(batting_club_id == conf$opponents_of_interest[[i]]) %>%
    slice_max(actuallyDate, n=conf$default_list_length) %>% 
    H.F.hyacinthfam("Recent opponents innings", H.hyacinth) %>%
    print()
  ...
  B.matches %>% filter(is_circle == TRUE ) %>% 
    ...
    slice_max(actuallyDate, n=conf$default_list_length) %>% 
    H.F.hyacinthfam("Recent matches between us", H.hyacinth) %>%
    print()
