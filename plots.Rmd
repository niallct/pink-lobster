...
player_name <- as.character(E.players %>% filter(player_id == conf$player_to_plot[[1]]) %>% 
                              slice_head(n=1) %>% select("Name"))

thisbatterty <- B.batting %>% filter(!is.na(Runs), batsman_id %in% conf$player_to_plot,
                                   Yr == conf$year_of_interest)

numbatsty <- nrow(thisbatterty)

thisbatter <- B.batting %>% filter(!is.na(Runs), batsman_id %in% conf$player_to_plot)

numbats <- nrow(thisbatter)

thisbowlerty <- B.bowling %>% filter(!is.na(O), bowler_id %in% conf$player_to_plot,
                                     Yr == conf$year_of_interest)
numbowlsty <- nrow(thisbowlerty)
thisbowler <- B.bowling %>% filter(!is.na(O), bowler_id %in% conf$player_to_plot)
...
---
subtitle: `r paste(player_name)`
---

<!-- `r paste("# Report for", player_name)` -->

This report contains, unless indicated, records for all clubs present in the
dataset.
...
### Averages
```{r }
B.batAvg %>% filter(batsman_id %in% conf$player_to_plot) %>%
H.F.plain("Career batting average", H.asterC)
```

### Performances
```{r bat-perfs}
thisbatter %>% slice_max(actuallyDate, n=2*conf$default_list_length) %>%
  select(all_of(H.comfrey)) %>% 
  kbl(longtable = T, booktabs = T, row.names = F, caption = "Recent batting performances") %>%
  kable_styling(latex_options = c("repeat_header", "striped", "hold_position"),
                font_size = 7) %>%
...
### Averages
```{r bowl-avgs}
B.bowlAvg %>% filter(bowler_id %in% conf$player_to_plot) %>%
H.F.plain("Career bowling average", H.clematisC)
```

### Performances
```{r bowl-perfs}
thisbowler %>% slice_max(actuallyDate, n=2*conf$default_list_length) %>%
  select(all_of(H.buttercup)) %>% 
  kbl(longtable = T, booktabs = T, row.names = F, caption = "Recent bowling performances") %>%
  kable_styling(latex_options = c("repeat_header", "striped", "hold_position"),
                font_size = 7) %>%
    column_spec(1, width = "8em") %>%
  column_spec(7, width = "8em") %>%
...
B.bowling %>% filter(bowler_id %in% conf$player_to_plot) %>%
  group_by(Yr) %>% 
