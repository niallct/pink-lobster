...
W.bat.avgsteam <- function(theteamname, theteam, df,
                           limit = 0.5 * conf$default_list_length) {  df %>%
...
W.bat.fiftiesteam <- function(theteamname, theteam, df,
                              limit = 0.5 * conf$default_list_length) {  df %>%
    filter(batting_team_id == theteam) %>%
    makeBatAvgs() %>% drop_na(Name) %>%
    # filter( Inns >= conf$qualify_inns_avgs)%>%
...
D.bat.fifties <- function(df, cap) {  df %>% 
    filter(Runs >= 50) %>%
    arrange(desc(Runs)) %>%
...
D.bat.fifties2 <- function(df, cap) {  df %>% 
    filter(Runs >= 50) %>%
    arrange(desc(Runs)) %>%
...
D.bat.milestone <- function(df, cap, ms = 50) {  df %>% 
    filter(Runs >= ms) %>%
    arrange(desc(Runs)) %>%
    select(all_of(H.lily)) %>%
    D.showtable(cap) %>% J.carrot
...
D.bat.fiftieslosing <- function(df, cap, ms = 50) {  df %>%
    filter(Runs >= ms & Res != "W") %>%
...
W.bat.msteam <- function(theteamname, theteam, df, ms = 50,
                         limit = conf$default_list_length) {  df %>%
...
D.bat.consecbatms <- function(df, cap, ms = 50) {  df %>%
    ...
    filter(milestone &
             (next_match_is_milestone |
                lag(milestone, default = FALSE))) %>%  # Keep both current and lag match if they are 100+
...
D.bat.most50season <- function(df, cap, ms = 50) {  df %>%
    filter(!is.na(Runs)) %>%
    group_by(batsman_id, Yr, Name) %>% 
    summarise(`Total` = sum(Runs >= ms), `Matches` = n()) %>% ungroup() %>%
    rename(Season = Yr) %>% select(-batsman_id) %>%
...
D.bat.outscoringotherteam <- function(dfinns, dfbat, cap,
                                      howmany = conf$default_list_length) {
  tobeat <- dfinns %>% filter(Total > 0, Res != "A") %>% 
    group_by(`actuallyDate`) %>%  slice_min(Total) %>%
    select(`Our Team`, actuallyDate, Total, Score) %>% ungroup()
  
  scores <- dfbat %>% filter(Name != 'Unsure') %>%
    select(Name, actuallyDate, Runs, `Our Team`, Date)
  
  full_join(tobeat, scores, by = 'actuallyDate') %>% filter(Runs > Total) %>%
    arrange(desc(actuallyDate)) %>%
    slice_max(actuallyDate, n = howmany, with_ties = F) %>%
    select(Name, `Batting for` = `Our Team.y`,
           Runs, `Team outscored` = `Our Team.x`, Score,  Date) %>%
    D.showtable(cap)
}
...
D.fow.pshipoveropp <- function(df, cap = "", top = 1000) {  df %>%
    filter(PartnershipRaw > opptotal, opptotal > 0) %>%
    arrange(desc(PartnershipRaw)) %>%
...
W.fow.pshiptopwkt <- function(df, thewkt = 1) {  df %>%
    filter(PartnershipRaw >= 25) %>%
    group_by(Wkt) %>% slice_max(PartnershipRaw, n = 5) %>%
...
W.bowl.bestavgbytype <- function(df, thistype = "League",
                                 min = conf$qualify_balls_bowled) {  df %>%
...
D.allround.keeperbowling <- function(df, cap) {  df %>%
    ...
    arrange(desc(W), R) %>%
    slice_max(W, n = conf$default_list_length, with_ties = F) %>%
    rename(`Fielding Side` = Club, `Batting Club` = Oppo, ) %>%
    select(all_of(H.cyclamen)) %>%
    D.showtable(cap) %>% J.parkin
}
...
D.bowl.mostwktseason <- function(df, cap) {  df %>%
    ...
    summarise(`Total` = sum(W), `Matches` = n()) %>% ungroup() %>%
...
D.bowl.bestavgseason <- function(df, cap, min = conf$qualify_inns_avgs) {  df %>%
    ...
    group_by(bowler_id, Yr, Name) %>%
    summarise(
      `Avg` = avRn(sum(R) / sum(W)),
      `Matches` = n(),  Wkts = sum(W) ) %>%
...
W.inns.quotients <- function(dfU, dfT) {
  ...
  a.Quots <- tribble (
    ...
    a.QRunsF,
    a.QWktsF,
    avRn(a.QRunsF / a.QWktsF),
    avRn(a.QRunsF / a.QInnsF),
    avRn(a.QWktsF / a.QInnsF),
    "Bowling",
    a.QInnsA,
    a.QRunsA,
    a.QWktsA,
    avRn(a.QRunsA / a.QWktsA),
    avRn(a.QRunsA / a.QInnsA),
    avRn(a.QWktsA / a.QInnsA)
  )
...
D.inns.manybowlers <- function(df, cap, ms = 10,
                               howmany = conf$default_list_length) {  df %>%
...
D.inns.allbowlersmaiden <- function(df, cap, 
                                    howmany = conf$default_list_length) {  df %>%
...
D.inns.equitbowl <- function(df, cap, tot = 250, mx = 50) {
  df %>%
...
D.ar.seasondouble <- function(dfA, dfB, dfC, cap, msA = 500, msB = 20, msC = 10) {
  full_join(
    dfA %>%
      group_by(batsman_id, Yr, Name) %>% summarise(
        `Total` = sum(Runs, na.rm = TRUE),
        `Matches` = n()
      ) %>% ungroup() %>%
      rename(Season = Yr) ,
    dfB %>%
    ...
    full_join(
      dfC %>%
        group_by(player_id, Yr, Name) %>% summarise(
          `Total` = sum(Dis, na.rm = TRUE),
          `Matches` = n()
        ) %>% ungroup() %>%
        rename(Season = Yr),
...
D.fielding.mostcatchesinns <- function(df, cap,
                                       howmany = conf$default_list_length,
                                       min = 2) {  df %>%
    arrange(desc(actuallyDate)) %>%
    filter(Ct >= min) %>%
...
D.fielding.mostcatchesinns2 <- function(df, cap,
                                        howmany = conf$default_list_length,
                                        min = 2) {  df %>%
    arrange(desc(actuallyDate)) %>%
    filter(Ct >= min) %>%
...
D.fieldsumm.mostcatches <- function(df,  cap,
                                    howmany = conf$default_list_length,
                                    min = 3) {  df %>%
    arrange(desc(Ct)) %>%
    filter(Ct >= min) %>%
...
D.fielding.mostblahinns <- function(df, cap, param, min = 2) {  df %>%
    ...
    slice_max(.data[[param]],
              n = conf$default_list_length,
              with_ties = F) %>%
    select(all_of(H.plum)) %>%
    D.showtable(cap) %>% J.lemon
}
...
D.bat.amesfreeman <- function(df, cap) {
  ...
  ) %>% select(all_of(H.begonia), fielder_name, bowler_name) %>%
...
D.inns.groundmostruns <- function(df, cap) {  df %>%
    ...
    ungroup() %>% arrange(-TRuns) %>% drop_na() %>% select(n = TRuns, Name =
                                                             Ground) %>%
    slice_max(n, n = conf$default_list_length) %>%
    select(all_of(H.mugwort)) %>%
    D.showtable(cap)
